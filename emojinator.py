...
async def emoji(message):
    ...
    if not content:
        try:
            ...
            await message.reply(__emoji_help)
        return
    if cmd == "import":
        out = []
        ...
        for arg,attachment in zip(args, message.attachments):
            ...
            try:
                ...
                await message.reply("!emoji failed on {}: {}".format(arg, e.text.split("\n")[1]))
                continue
            out.append(str(emoji))
        await message.reply(get_emoji_count(message.guild) + "\n" + " ".join(out))
    if cmd == "url":
        name, url = args
        ...
        try:
            ...
            await message.reply("!emoji failed: {}".format(e.text))
            return
        await message.reply(get_emoji_count(message.guild) + "\n" + str(emoji))
    elif cmd == "geturl":
        out = []
        ...
        await message.reply("{1} = https://pbs.twimg.com/media/{0}.jpg".format(*mashup))
...
#        await client.logout()
    ...
    if message.content.lower().startswith("!emoji"):
        ...
        if message.content.lower().startswith("!emoji mashup") or message.author.permissions_in(message.channel).manage_emojis:
            ...
            await message.reply("No 'Manage Emoji' permission")

    if message.content.lower() == "!invite":
        await message.reply(discord.utils.oauth_url(client.user.id, permissions=discord.Permissions(1073743872)))
